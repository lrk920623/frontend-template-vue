...
import VoPage from 'components/basic/Page'
import VoSelectList from 'components/modal/sub/SelectList'
import VoSpin from 'components/basic/Spin'
...
export default {
  ...
  watch: {
    ...
    record(val) {
      ...
      this.list = d
      this.total = _.isArray(val) ? val.length : val.total || val.count || 0
    }
  },
  ...
  methods: {
    ...
    selectRemove(remove) {
      ...
      _.remove(this.selectList, n => n[this.type.name] === remove)
    ...
    choose(curr) {
      ...
      if (this.single) {
        ...
        curr.selected
          ...
          : _.remove(
              this.selectList,
              n => n[this.type.code] === content[this.type.code]
            )
      }
